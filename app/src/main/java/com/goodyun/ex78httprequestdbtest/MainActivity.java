...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //외부저장소 읽고 쓰기 권한 퍼미션..체크및 요청

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if(checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE)== PackageManager.PERMISSION_DENIED){
                //허용이 안되어있는 상태이므로.. 퍼미션요청 다이얼로그 보이기..
        ...
        switch (requestCode){
            case 100:
                if(grantResults[0]==PackageManager.PERMISSION_DENIED){
                    Toast.makeText(this, "이미지 선택이 불가합니다.", Toast.LENGTH_SHORT).show();
                }
    ...
    public void clickUpload(View v){
        ...
        String serverUrl="http://toutt123.dothome.co.kr/android/insertDB.php";
        String name = editName.getText().toString();
        String msg = editMsg.getText().toString();

        //insertDB.php에 요청한 결과가 String으로 받으려면 StringRequest객체 생성 ..
